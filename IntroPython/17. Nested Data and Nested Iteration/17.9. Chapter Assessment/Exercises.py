...
#3. Below, we’ve provided a list of lists. Use in statements to create variables with
...
# Check to see if the string data is a key in nested, if it is, assign True to the variable data, otherwise
# assign False.
if "data" in nested:
    data = True
else:
...
# Check to see that the string 'whole' is not in the value of the key window. If it's not, then assign to
# the variable whole the value of True, otherwise False.
if "whole" in nested:
    whole = True
else:
    whole = False

# Check to see if the string 'physics' is a key in the dictionary nested. If it is, assign to the variable
# physics, the value of True, otherwise False.
if "physics" in nested:
    physics = True
else:
    physics = False

#5. The variable nested_d contains a nested dictionary with the gold medal counts for the top four countries
# in the past three Olympics. Assign the value of Great Britain’s gold medal count from the London Olympics
...
for city in ["Beijing", "London", "Rio"]:
    US_count.append(nested_d[city]["USA"])
...
#9. Given below is a list of lists of athletes. Create a list, t, that saves only the athlete’s name if it
